...
main = do
    ...
    -- let tokens = [("id", expr_id), ("num", expr_num), ("op_arit", expr_op_a), ("asign", expr_asig), ("op_rel", expr_op_rel), ("res", expr_res), ("punt", expr_punt), ("delim", expr_delim)]
    ...
    -- Mostrar cada AFD (transiciones y finales) antes de construir la MDD
    mapM_ (\(name, afd) -> do
            putStrLn $ "=== AFD para token: " ++ name ++ " ==="
            putStrLn $ "Estados: " ++ show (estadosD afd)
            putStrLn $ "Alfabeto: " ++ show (alfabetoD afd)
            putStrLn $ "Transiciones: "
            mapM_ print (transicionesD afd)
            putStrLn $ "Finales: " ++ show (finalesD afd)
            -- detectar transiciones duplicadas (mismo (estado, simbolo) -> múltiples destinos)
            let pairs = [ ((q,c), qn) | (q,c,qn) <- transicionesD afd ]
